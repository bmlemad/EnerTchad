...
;
try{(function(){var red=matchMedia('(prefers-reduced-motion: reduce)').matches;var prevCxl=null;function settle(id,pre){if(prevCxl)prevCxl();var el=document.getElementById(id);if(!el)return;var H=document.documentElement,pv=H.style.scrollBehavior;var stop=false;var preW=pre||0;var ownCorr=0;var armT=performance.now();function cxl(){stop=true;H.style.scrollBehavior=pv;removeEventListener('scroll',onExt);}prevCxl=cxl;function onExt(){if(ownCorr>0){ownCorr--;return;}if(preW>0&&performance.now()-armT<preW){preW=0;return;}cxl();}addEventListener('wheel',cxl,{passive:true,once:true});addEventListener('touchstart',cxl,{passive:true,once:true});addEventListener('keydown',cxl,{once:true});addEventListener('pointerdown',cxl,{passive:true,once:true});addEventListener('scroll',onExt,{passive:true});var tries=0;var last=null;function step(){if(stop)return;var t=el.getBoundingClientRect().top;if(last!==null&&Math.abs(t-last)<2){cxl();return;}last=t;if(tries<8){tries++;if(Math.abs(t)>28){ownCorr++;H.style.scrollBehavior='auto';el.scrollIntoView({block:'start'});}setTimeout(step,tries<4?120:250);}else{cxl();}}setTimeout(step,red?60:750);}addEventListener('hashchange',function(){var id=location.hash.slice(1);if(id)settle(id,0);});document.addEventListener('click',function(e){var a=e.target.closest&&e.target.closest('a[href^="#"]');if(!a)return;var id=a.getAttribute('href').slice(1);if(id)settle(id,250);},true);if(location.hash.length>1){setTimeout(function(){settle(location.hash.slice(1),1500);},300);}})();
}catch(_e){/* section absente sur cette page */}
;
